...
const id = me.getAttribute("id");
const parameters = me.getAttribute("parameters")
const origen = me.getAttribute("origen");
//var token = me.getAttribute("rol");
let color1 = me.getAttribute("color");
const animationDuration = parseInt(me.getAttribute("animationDuration"));
const posBottom = me.getAttribute("position");
const server = "beta-sacmed.novacaribe.com";
const visibility = eval(me.getAttribute("visibility"));
const buttonVisibility = eval(me.getAttribute("buttonVisibility"));
const windowWidth = me.getAttribute("windowWidth");
const windowHeight = me.getAttribute("windowHeight");
//var url_env = ""; // LOCAL
...
settings.right = '20px';
settings.width = windowWidth + 'px'; // Default 360
settings.height = windowHeight + 'px'; // Default 570
...
const SACMED_ROL = {
  minimizeROL: function () {
    if (window.mobileAndTabletcheck()) {
      window.open(settings.url + id + pars, '_blank');
      return;
    }
  ...
  setRol: function () {
    ...
    if (ifr == null) {
      ...
      else if (posBottom === 'center')
        rol_div.style.left = (window.innerWidth / 2 - parseInt(settings.width) / 2).toString() + "px";
      else
      ...
      if (pars === '')
        ...
        iframe.src = settings.url + settings.id + pars;
      ...
      svg.innerHTML = '<svg id="Capa_2" xmlns="http://www.w3.org/2000/svg" height="50pt" width="50pt" x="0px" y="0px" viewBox="0 0 612 792" style="enable-background:new 0 0 612 792;" xml:space="preserve"><style type="text/css">.st0{fill:color1;}.st1{fill:#FFFFFF;}.st3{font-family:\'Helvetica-Bold\',serif; font-weight: bold;}.st4{font-size:161.2966px;}</style><circle class="st0" cx="306" cy="396" r="256"/><path class="st1" d="M229.66,313.02L229.66,313.02c-11.77,0-21.32-9.55-21.32-21.32v-25.17c0-11.77,9.55-21.32,21.32-21.32h0c11.77,0,21.32,9.55,21.32,21.32v25.17C250.98,303.48,241.44,313.02,229.66,313.02z"/><path class="st1" d="M383.12,313.02L383.12,313.02c-11.77,0-21.32-9.55-21.32-21.32v-25.17c0-11.77,9.55-21.32,21.32-21.32h0c11.77,0,21.32,9.55,21.32,21.32v25.17C404.44,303.48,394.9,313.02,383.12,313.02z"/><path class="st1" d="M419.53,268.98H192.06c-16.56,0-29.98,13.42-29.98,29.98v193.79c0,16.56,13.42,29.98,29.98,29.98h227.47c16.56,0,29.98-13.42,29.98-29.98V298.96C449.51,282.41,436.09,268.98,419.53,268.98z M229.66,334.69c-15.35,0-27.79-12.44-27.79-27.79c0-15.35,12.44-27.79,27.79-27.79c15.35,0,27.79,12.44,27.79,27.79C257.45,322.25,245.01,334.69,229.66,334.69z M383.12,334.69c-15.35,0-27.79-12.44-27.79-27.79c0-15.35,12.44-27.79,27.79-27.79c15.35,0,27.79,12.44,27.79,27.79C410.91,322.25,398.47,334.69,383.12,334.69z"/><text class="st0 st3 st4" x="50%" y="60%" text-anchor="middle">_dia_</text></svg>'.replace('color1', color1).replace('_dia_', day);
      ...
      if (posBottom == null || posBottom === '' || posBottom === 'right')
        ...
        button.style.left = (window.innerWidth / 2 - parseInt(settings.width) / 2).toString() + "px";
      else
        button.style.left = "0px";

      if (visibility == null || visibility === true && buttonVisibility === true)
        button.style.display = "block";
      else
        button.style.display = "none";
